/* ============================================================
   A CASCA DAS PÁGINAS QUE O GOOGLE LÊ
   ------------------------------------------------------------
   O site inteiro é uma página só, e tudo o que é publicado vive
   atrás de um "#" no endereço. O Google não lê nada depois do
   "#": ele indexa a capa e mais nada.

   Por isso existem páginas montadas no servidor — uma para cada
   material (/m/<id>) e uma para cada aula (/a/<id>). São duas
   páginas iguais por fora, então o desenho, o escapamento e as
   contas de dinheiro moram aqui, num arquivo só.

   O nome começa com "_" de propósito: a Vercel entende que este
   arquivo é peça de dentro, não um endereço do site.
   ============================================================ */

export const SUPABASE_URL = 'https://pmpwkunsyuanrazrmomb.supabase.co';
export const CHAVE = 'sb_publishable_eSh0TNQPFfJEb15Oxl6ZIQ_1TA0w-Ko';
export const SITE = 'https://www.aprenderconecta.com.br';
...
export function jsonSeguro(obj) {
...
/* ------------------------------------------------------------------
   ...
   quando o endereço passa pela regra de reescrita (/m/<id>, /a/<id>),
   esse campo às vezes chega vazio, ou chega com o mesmo valor duas
   vezes (uma da rota, outra da query) — e aí vira uma lista, não um
   texto.

   Então não confiamos em um caminho só: tentamos os três.
   ------------------------------------------------------------------ */
export function lerId(req, letra) {
  ...
  try {
    ...
    const noCaminho = u.pathname.match(new RegExp('/' + letra + '/([^/?#]+)'));
...
export function idParece(id) {
  return /^[0-9a-fA-F-]{20,40}$/.test(id);
}

/* pergunta ao banco; devolve { status, corpo } sem estourar */
export async function perguntar(caminho) {
...
export function pagina({ titulo, descricao, capa, corpo, url, robots }) {
  return `<!doctype html>
<html lang="pt-BR">
<head>
<meta charset="utf-8">
...
/* A página de "não achei", igual para material e para aula.
   `chamada` já vem escrita por inteiro ("Material não encontrado",
   "Aula não encontrada") porque em português o encontrado muda de
   gênero, e montar isso com um "(a)" no meio ficava feio na tela. */
export function naoAchei(res, { chamada, motivo, voltar, rotuloVoltar }) {
  res.status(404).send(pagina({
    titulo: `${chamada} — AprenderConecta`,
    descricao: 'Este endereço não existe mais no AprenderConecta.',
    capa: `${SITE}/capa-compartilhar.png`,
    url: `${SITE}${voltar}`,
    robots: 'noindex, follow',
    corpo: `<h1>${esc(chamada)}</h1>
      <p class="por">${esc(motivo)}</p>
      <a class="btn" href="${SITE}${voltar}">${esc(rotuloVoltar)}</a>`,
  }));
}
